...
* **不存在全面胜者。** SAGEE 准确率高 2.7 个点且几乎不弃权（1/1916），
  但 MacroF1 低 12.8 个点 —— 它在小类上崩得很厉害（`StudyRoom` **0.000**）。
* **分界线是"这个类型能不能靠几何/拓扑推出来"**：
  * 能 → SAGEE：`Corridor` 0.837（大模型 0.576）、`Bathroom` 0.823、`ElevatorShaft` 0.797、
    `ElectricalRoom` 0.550；
  * 不能 → 大模型：`Cloakroom` 0.769（SAGEE 0.333）、`Stairwell` 0.947、`Garden` 0.714、
    `Kitchen` 0.832、`StudyRoom` 0.825。
